                ...
                <>
                    <section className="homeContainer">
                        <div className="homeSection">
                            <Carousel className="carousel-container" autoPlay infiniteLoop={true} style={{ borderRadius: "20px" }} showThumbs={false} showArrows={true}>
                                {
                                    addTask.length > 0 ? addTask.map((e, i) => {
                                        return (
                                            <div className="carousel-data-container">
                                                <img src={e.image} alt={i} style={{width: "100%", opacity: "0.5"}} />
                                                <div className="img-cover" style={{width: "100%", height: "550px", backgroundColor: "rgba(0,0,0,0.5)",
                                                marginTop: "-550px"
                                                }}><p className="legend-carousel">{e.title}<br /><span>{`By ${e.author.name}`}</span></p>
                        ...
                        <div className="allBlogs">
                            <div className="leftBlogSection">
                                {addTask.length > 0 ? addTask.map((e, i) => {
                                    ...
                                    return (
                                        <div className="col4gy3row02" key={e._id + 1} onMouseEnter={()=> {
                                            var arr = [...hoverClass];
                                            ...
                                            <div className="colis1002">
                                                ...
                                                >
                                                    ...
                                                    <div className="authHeading02 ">
                                                        <p className={`authorTitle02 ${hoverClass[i]?.hoverClass ? "lp" : "no-lp"}`}>{e.title}</p>
                                                        <p className={`authSubHed02 ${hoverClass[i]?.hoverClass ? "lp" : "no-lp"}`}>{shortenedBody}</p>
                                                    </div>
                                                </Link>
                                                <div className={`authDaTiSt02 ${hoverClass[i]?.hoverClass ? "lp" : "no-lp"}`}>
                                                    <span>{e.userBlogDate} ·</span>
                                                    <span className="">{readingDuration}</span>
                                                    <span className="mx-1 ">·</span>
                                                    <span className={`mx-1 ${hoverClass[i]?.hoverClass ? "userTagHover" : "userBlogTag"}`}>{e.tags[0].name}</span>
                                                    <span className="mx-1 ">&#9733;</span>
                                                </div>
                                            </div>
                                ...
                                <div className="allBlogTypes">
                                    <div className="allBlogTypesFirst">
                                        ...
                                        <div className="allspanTag">
                                            {
                                                isLoading ? <></> : tags.slice(0, 9).map((e, i) => {
                                                    return (<a style={{ textDecoration: "none" }}
                                                        href={`http://localhost:3000/tags/${e.name}`} key={e._id + 2}>{e.name}</a>)
                                                })
                                            }
                                        </div>
                                        <a className="allTopics" href="http://localhost:3000/tags">See all tags</a>
                                    </div>
